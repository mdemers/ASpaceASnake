...
def get_all_resources(tar_dir,repo_num):
    all_ids = client.get('repositories/' + repo_num + '/archival_objects?all_ids=true').json()
    for resource_id in all_ids:
        newJSON = tar_dir + "/" + str(resource_id) + '.json'
        resource = client.get('repositories/' + repo_num + '/archival_objects/' + str(resource_id)).json()
